...
public class CheckCollectButton {
    ...
    public CheckCollectButton(float x, float y, String text) {
        this.hb.move(x, y);
        this.current_x = x;
        this.current_y = y;
        this.text = text;
    ...
    public void render(SpriteBatch sb) {
        renderButton(sb);
        if (FontHelper.getSmartWidth(FontHelper.buttonLabelFont, this.text, 9999.0F, 0.0F) > 200.0F * Settings.scale) {
            FontHelper.renderFontCentered(sb, FontHelper.buttonLabelFont, this.text, this.current_x, this.current_y,
                    this.textColor, 0.8F);
        } else {
            FontHelper.renderFontCentered(sb, FontHelper.buttonLabelFont, this.text, this.current_x, this.current_y,
                    this.textColor);
    ...
    private void renderButton(SpriteBatch sb) {
        ...
        if (Settings.isControllerMode) {
            if (this.controllerImgTextWidth == 0.0F)
                this.controllerImgTextWidth = FontHelper.getSmartWidth(FontHelper.buttonLabelFont, this.text, 99999.0F,
                        0.0F) / 2.0F;
